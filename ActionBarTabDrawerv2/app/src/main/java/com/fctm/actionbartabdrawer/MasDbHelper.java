...
public class MasDbHelper {
    ...
    public MasDbHelper(Context context) {
        ...
        helper = new DBHelper(context, "Masfile", null, 1);
    }
    ...
    // 新增一筆新的成績
    public void add(String sch, String timebeg, String timeend, String workitem) {
        // 取出資料庫物件, 並且是可以寫入狀態
        // 當APP空間不夠時, 該方法會丟出例外
    ...
    // 更新單字
    public void update(String sch, Vocabulary v) {
        // 取出資料庫物件, 並且是可以寫入狀態
        // 當APP空間不夠時, 該方法會丟出例外
        SQLiteDatabase db = helper.getWritableDatabase();
        ...
        // 更新資料到資料表(Table)
        db.update("mas", values, "sch='" + sch + "'", null);
    ...
    public Vocabulary get(String sch) {
        ...
        // 執行查詢
        Cursor c = db.query("mas", new String[]{"_id, sch, timebeg, timeend", "workitem"},
    ...
    public int getCount() {
        ...
        Log.d("VocabluaryDBHelper", "getCount=" + count);
    ...
    public ArrayList<Vocabulary> getAllData() {
        ...
        // 透過query來查詢資料
        Cursor c = db.query("mas",                           // 資料表名字
                new String[]{"_id", "sch", "timebeg", "timeend", "workitem"},  // 要取出的欄位資料
                null,                                        // 查詢條件式
                null,                                        // 查詢條件值字串陣列
                null,                                        // Group By字串語法
                null,                                        // Having字串法
        ...
        Log.d("Rank", "Query:" + c.getCount());

        while (c.moveToNext()) {
            Vocabulary d = new Vocabulary();
            d.id = c.getLong(c.getColumnIndex("_id"));    // 取出名字欄位資料
            d.sch = c.getString(c.getColumnIndex("sch"));    // 取出名字欄位資料
            d.timebeg = c.getString(c.getColumnIndex("timebeg"));    // 取出名字欄位資料
            d.timeend = c.getString(c.getColumnIndex("timeend"));     // 取出翻卡片次數資料
    ...
    public void delete(String sch) {
        SQLiteDatabase db = helper.getReadableDatabase();
        db.delete("mas", "sch='" + sch + "'", null);
        db.close();
    }
    ...
    public boolean isExisting(String sch) {
        ...
        // 執行查詢
        Cursor c = db.query("mas", new String[]{"sch"},
                "sch='" + sch + "'", null, null, null, null, null);
